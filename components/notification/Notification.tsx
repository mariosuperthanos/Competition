...
export function Notification({ id, title, message, date, isSeen, purpose }: NotificationProps) {
  ...
  const interactiveFunc = async () => {
    ...
    try {
      ...
      const URL = `${window.location.origin}/api/create-notification`;
      const changeButtonState = await axios.post(`${window.location.origin}/api/buttonStateOnName`, { title, clientName: name, buttonState: response, id }, {
        headers: {
